"""Performance monitoring for database operations with timing decorators."""
...
class PerformanceMonitor:
    """Enhanced performance monitoring for database operations with timing decorators."""
    
    # Sampling threshold - use sampling when metrics exceed this count
    _SAMPLING_THRESHOLD = 1000
    # Sampling rate when threshold is exceeded
    _SAMPLING_RATE = 0.1  # 10% sampling
    ...
    def record_operation(self, operation_name: str, duration: float, success: bool = True) -> None:
        """Record a database operation with timing and success status."""
    ...
    def time_operation(self, operation_name: str) -> Callable:
        """Decorator to time synchronous operations and record performance metrics.
    ...
    def time_async_operation(self, operation_name: str) -> Callable:
        """Decorator to time asynchronous operations and record performance metrics.
    ...
    def time_method(self, operation_name: Optional[str] = None) -> Callable:
        """Flexible decorator that works with both sync and async methods.
    ...
    def get_stats(self, use_cache: bool = True) -> Dict[str, Any]:
        """Get comprehensive performance statistics including concurrency metrics.
    ...
    def _calculate_all_stats(self, now: float) -> Dict[str, Any]:
        ...
        for metric_name, values in self.metrics.items():
            if values:
                ...
                if len(values_list) > self._SAMPLING_THRESHOLD:
                    # Sample 10% of values for large datasets
                    sample_size = max(int(len(values_list) * self._SAMPLING_RATE), 2)
    ...
    def _calculate_metric_stats(self, values: Sequence[float]) -> Dict[str, float]:
        ...
        Returns:
            Dictionary with count, avg, min, max, total, std_dev
        """
        values_list = list(values)
        n = len(values_list)
    ...
    def _welford_stats(self, values: List[float]) -> tuple:
        ...
        # This is more numerically stable than the naive formula
    ...
    def _calculate_std_dev(self, values: Sequence[float]) -> float:
        """Calculate standard deviation for performance metrics.
        
        Note: This method is kept for backward compatibility.
        Consider using _calculate_metric_stats for full statistics.
        ...
        _, variance, _, _, _ = self._welford_stats(values_list)
    ...
    def log_summary(self, logger: logging.Logger) -> None:
        ...
        logger.info(f"Performance Monitor Summary (uptime: {stats['uptime']:.1f}s)")
    ...
    def log_detailed_summary(self, logger: logging.Logger) -> None:
        """Log detailed performance summary with all available metrics."""
        stats = self.get_stats()
        logger.info("=== Detailed Performance Monitor Summary ===")
        ...
        for op_name in stats['operations']:
            ...
            if duration_key in stats:
                ...
                logger.info(f"  Duration - avg: {duration_stats['avg']:.6f}s, min: {duration_stats['min']:.6f}s, max: {duration_stats['max']:.6f}s")
                logger.info(f"  Duration - total: {duration_stats['total']:.6f}s, std_dev: {duration_stats['std_dev']:.6f}s")
        ...
        logger.info("=== End Detailed Performance Summary ===")
