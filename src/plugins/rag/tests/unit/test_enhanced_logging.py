...
from src.plugins.rag.infrastructure.logging import (
    RagError, ConfigurationError, NetworkError, DatabaseError,
    ExternalServiceError, SearchError, RetrievalError, GradingError,
    InjectionError, ValidationError, ResourceError,
...
class TestLoggingUtils:
    ...
    @patch('src.plugins.rag.infrastructure.logging.LoggingUtils.get_rag_logger')
    def test_log_structured_error(self, mock_get_logger):
        """Test structured error logging."""
        mock_logger = MagicMock()
        mock_get_logger.return_value = mock_logger

        error = ConfigurationError("Test config error")
        LoggingUtils.log_structured_error(error, mock_logger)

        # Verify logger was called with structured message
        mock_logger.log.assert_called_once()
        call_args = mock_logger.log.call_args
        assert call_args[0][0] == 40  # ERROR level
        assert "Structured Error:" in call_args[0][1]
        assert "ConfigurationError" in call_args[0][1]
